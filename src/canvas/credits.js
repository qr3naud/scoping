  ...
  window.__cbCanvasModules.createCreditHelpers = function createCreditHelpers(deps) {
    ...
    function notifyCreditTotal() {
      ...
      for (const c of cardsRef()) {
        ...
        if (!c.data.usePrivateKey) creditTotal += c.data.credits ?? 0;
        actionExecTotal += c.data.actionExecutions ?? 0;
      }
      if (window.__cb.updateCreditTotal) window.__cb.updateCreditTotal(creditTotal, actionExecTotal);
    }
